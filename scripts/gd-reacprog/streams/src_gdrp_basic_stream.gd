...
func unsubscribe(subscriber : GDRP_Subscriber) -> GDRP_Stream:
	...
	if _link_connections.has(subscriber):
		subscriber.disconnect("notified", _link_connections[subscriber])
		_link_connections.erase(subscriber)
	return self

func link_to(what : GDRP_Subscriber) -> GDRP_Stream:
	_link_connections[what] = func(n): 
		if n == NOTIFICATION_PREDELETE: unsubscribe(what)
	what.connect("notified", _link_connections[what])
	return self
